-- v4: Xclusive Club & Kitchen table layout (see /public — poster "TABLE LAYOUT")
-- Replaces the old Diamond/Platinum/Gold/Silver/VIP/Standing categories with the
-- venue's actual zones, adds `sort_order` so the grid always renders in poster order,
-- and aligns the minimum-bid-increment with lib/bidding-constants.ts (BID_INCREMENT).
--
-- Run this once in the Supabase SQL Editor for the NEW project
-- (https://uuaodaetiqgquigntfot.supabase.co -> SQL Editor -> paste -> Run).
-- Safe to re-run: table/function creation is idempotent and the seed uses
-- ON CONFLICT (id) DO UPDATE, so re-running just resets prices/zones, not bid history.
...
CREATE TABLE IF NOT EXISTS tables (
  ...
  bidding_ends_at TIMESTAMP WITH TIME ZONE DEFAULT (NOW() + INTERVAL '3 days'),
  created_at TIMESTAMP WITH TIME ZONE DEFAULT NOW(),
  updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW()
);

-- Add sort_order if migrating an existing v3 `tables` table instead of creating fresh
...
-- place_bid(): atomic bid placement with row locking + optimistic-version check.
-- Minimum increment is BID_INCREMENT (₹1,000) — keep this in sync with
-- lib/bidding-constants.ts if you ever change the increment.
...
BEGIN
    -- Plain NOW() (timestamptz). `NOW() AT TIME ZONE 'Asia/Kolkata'` yields an IST
    -- wall-clock *without* zone, which Postgres then re-reads as UTC — shifting "now"
    -- 5.5h into the future and rejecting bids in short windows as BIDDING_ENDED.
    v_current_time := NOW();
...
-- Seed: 38 tables from the Xclusive Club & Kitchen poster layout, in poster order
-- (sort_order). Only XC1-6 (₹100,000) and A1-6 (A1-3 ₹100,000, A4-6 ₹80,000) are open
-- for bidding (is_active = true below); every other table is a non-biddable landmark.
-- Re-running only resets zone/price/order/pax/is_active, never bid history.
INSERT INTO tables (id, name, category, pax, sort_order, base_price, current_bid) VALUES
('XC1', 'XC1', 'DJ_BOOTH', '6-8', 1, 100000, 100000),
('XC2', 'XC2', 'DJ_BOOTH', '6-8', 2, 100000, 100000),
('XC3', 'XC3', 'DJ_BOOTH', '6-8', 3, 100000, 100000),
('XC4', 'XC4', 'DJ_BOOTH', '6-8', 4, 100000, 100000),
('XC5', 'XC5', 'DJ_BOOTH', '6-8', 5, 100000, 100000),
('XC6', 'XC6', 'DJ_BOOTH', '6-8', 6, 100000, 100000),
('E1', 'E1', 'LEVEL_01', '4-6', 7, 10000, 10000),
('E2', 'E2', 'LEVEL_01', '4-6', 8, 10000, 10000),
('E3', 'E3', 'LEVEL_01', '4-6', 9, 10000, 10000),
('T1', 'T1', 'RESERVED', '2-4', 10, 10000, 10000),
('D1', 'D1', 'DANCE_FLOOR', '4', 11, 10000, 10000),
('D2', 'D2', 'DANCE_FLOOR', '4', 12, 10000, 10000),
('D3', 'D3', 'DANCE_FLOOR', '4', 13, 10000, 10000),
('D4', 'D4', 'DANCE_FLOOR', '4', 14, 10000, 10000),
('D5', 'D5', 'DANCE_FLOOR', '4', 15, 10000, 10000),
('D6', 'D6', 'DANCE_FLOOR', '4', 16, 10000, 10000),
('D7', 'D7', 'DANCE_FLOOR', '4', 17, 10000, 10000),
('D8', 'D8', 'DANCE_FLOOR', '4', 18, 10000, 10000),
('D9', 'D9', 'DANCE_FLOOR', '4', 19, 10000, 10000),
('D10', 'D10', 'DANCE_FLOOR', '4', 20, 10000, 10000),
('F1', 'F1', 'FLOOR_SIDE', '4', 21, 10000, 10000),
('F2', 'F2', 'FLOOR_SIDE', '4', 22, 10000, 10000),
('F3', 'F3', 'FLOOR_SIDE', '4', 23, 10000, 10000),
('F4', 'F4', 'FLOOR_SIDE', '4', 24, 10000, 10000),
('F5', 'F5', 'FLOOR_SIDE', '4', 25, 10000, 10000),
('A1', 'A1', 'FRONT_ROW', '6-8', 26, 100000, 100000),
('A2', 'A2', 'FRONT_ROW', '6-8', 27, 100000, 100000),
('A3', 'A3', 'FRONT_ROW', '6-8', 28, 100000, 100000),
('A4', 'A4', 'FRONT_ROW', '6-8', 29, 80000, 80000),
('A5', 'A5', 'FRONT_ROW', '6-8', 30, 80000, 80000),
('A6', 'A6', 'FRONT_ROW', '6-8', 31, 80000, 80000),
('B1', 'B1', 'BALCONY', '4-6', 32, 10000, 10000),
('B2', 'B2', 'BALCONY', '4-6', 33, 10000, 10000),
('B3', 'B3', 'BALCONY', '4-6', 34, 10000, 10000),
('KC1', 'KC1', 'KEY_CLUB', '8-10', 35, 10000, 10000),
('KC2', 'KC2', 'KEY_CLUB', '8-10', 36, 10000, 10000),
('T2', 'T2', 'RESERVED', '2-4', 37, 10000, 10000),
('T3', 'T3', 'RESERVED', '2-4', 38, 10000, 10000)
...
-- Only XC1-6 and A1-6 are open for bidding. Reset them to their fresh starting price
-- (clearing any earlier test bids) and activate them; deactivate everything else so
-- they stop showing up in the API (bid history, if any, is kept either way).
UPDATE tables SET
...
) AS EXCLUDED(id, base_price)
WHERE tables.id = EXCLUDED.id;
...
-- Needed because the seeding script (and login, if you ever self-serve signup) run
-- with the anon/publishable key, not a service-role key which would bypass RLS.
...
-- Enable Realtime (instant push updates) for the bidding tables. On a fresh Supabase
-- project the `supabase_realtime` publication exists but has no tables in it, so without
-- this the app still works (it falls back to polling) but bids won't feel instant.
DO $$
BEGIN
  IF NOT EXISTS (
  ...
  END IF;
  IF NOT EXISTS (
    SELECT 1 FROM pg_publication_tables WHERE pubname = 'supabase_realtime' AND tablename = 'bids'
  ) THEN
